...
config = read.config(os.path.join(_SCRIPT_DIR, '..', 'configs', 'single_9kpt.yaml'))
...
# For mouse9: center on SpineF (3), rotate heading toward Snout (0)
# (TailBase=4 and SpineF=3 can overlap in some frames causing NaN)
nan_mask = np.isnan(pose).any(axis=(1, 2))
if nan_mask.any():
    print(f"  WARNING: {nan_mask.sum()} NaN frames detected, interpolating...")
...
pose = preprocess.rotate_spine(preprocess.center_spine(pose, keypt_idx=3), keypt_idx=[3, 0])
...
rel_vel, rel_vel_labels = features.get_velocities(
    ...
    joints=np.delete(np.arange(9), 3),
    widths=[5, 11, 51],
    f_s = 30
)
